...
fn test_close_task() {
    ...
    assert_eq!(closed["status"].as_str().unwrap(), "done");
...
fn test_mcp_list_tasks_no_namespace_returns_all() {
    ...
    client.call_tool("create_task", serde_json::json!({"title": "Default task"}));
    client.call_tool(
        "create_task",
...
fn test_unlink_tasks() {
    ...
    let link_id = extract_content(&link_resp)["link_id"]
        .as_str()
        .unwrap()
        .to_string();
    ...
    let bad_resp = client.call_tool(
        "unlink_tasks",
        serde_json::json!({"link_id": "nonexistent"}),
    );
